...
MODEL_PATH = 'models/banana_classifier_best_model.keras'

# Model URL from GitHub Releases
MODEL_URL = "https://github.com/iamchaarles/Banalyzer/releases/download/v1.0.0/banana_classifier_best_model.keras"
...
if model is None:
    ...
    **Setup Instructions:**
    ...
    1. Upload your trained model (`best_model.keras`) to cloud storage:
       - **Google Drive**: Upload file → Get shareable link → Replace FILE_ID in code
       - **Hugging Face**: Create space → Upload model → Copy download URL
       - **Dropbox**: Upload file → Get direct download link
    
    2. Update `MODEL_URL` in the code (line 58) with your model's URL
    
    3. Redeploy your Streamlit app
    
    **Alternative**: If your model is < 100MB, use Git LFS:
    ```bash
    git lfs track "models/*.keras"
    git add .gitattributes models/best_model.keras
