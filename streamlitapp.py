...
# GitHub Release URLs
DETECTOR_URL = "https://github.com/iamchaarles/Banalyzer/releases/download/V2.0.0/banana_detector_final.keras"
CLASSIFIER_URL = "https://github.com/iamchaarles/Banalyzer/releases/download/V2.0.0/banana_classifier_best_model.keras"
...
def load_detector():
    ...
    model_path = download_model(DETECTOR_URL, DETECTOR_PATH)
...
def load_classifier():
    ...
    # Download if not exists
    model_path = download_model(CLASSIFIER_URL, CLASSIFIER_PATH)
...
if classifier is None:
    st.error("⚠️ Failed to load ripeness classifier. Please try again.")
    st.stop()

# Show detector status (Only show warning if missing, no success message if present)
if detector is None:
...
if uploaded_file is not None:
    ...
    if st.button("🔍 Analyze Ripeness"):
        ...
        if detector is not None:
            with st.spinner("Checking for banana..."):
